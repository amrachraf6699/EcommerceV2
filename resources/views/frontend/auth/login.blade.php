@php($title = __('storefront.auth.login'))
...
<section class="px-6 pb-20" style="padding-top:120px">
  <div class="max-w-2xl mx-auto border p-8 md:p-10" style="border-color:var(--line-soft);background:var(--gray-dark)">
    <div class="mb-8 text-center">
      ...
      <h1 class="text-3xl font-black mb-3">{{ __('storefront.auth.login') }}</h1>
      <p style="color:var(--gray-light)">{{ __('storefront.auth.subtitle') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('storefront.auth.login.store') }}" class="space-y-5">
      @csrf
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <label class="flex items-center gap-3 text-sm" style="color:var(--gray-light)">
          <input type="checkbox" name="remember" value="1" style="accent-color:white" @checked(old('remember'))>
          {{ __('storefront.auth.remember') }}
        </label>
        <a href="{{ route('storefront.auth.password.request') }}" style="color:var(--white)">{{ __('storefront.auth.forgot_password') }}</a>
      </div>

      <button type="submit" class="btn-primary w-full"><span>{{ __('storefront.auth.login') }}</span></button>
    </form>

    <p class="mt-6 text-center text-sm" style="color:var(--gray-light)">
      {{ __('storefront.auth.no_account') }}
      <a href="{{ route('storefront.auth.register') }}" style="color:var(--white);font-weight:700">{{ __('storefront.auth.create_account') }}</a>
    </p>
  </div>
</section>
@endsection
